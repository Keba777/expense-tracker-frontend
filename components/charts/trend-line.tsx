...
const CustomTooltip = ({ active, payload, label, incomeLabel, expenseLabel }: any) => {
    ...
    <div className="surface-1 rounded-xl px-3 py-2 shadow-card text-xs space-y-1">
      ...
      {payload.map((p: any) => (
        <p key={p.name} style={{ color: p.color }}>
          {p.name === "income" ? incomeLabel : expenseLabel}: {formatCompact(p.value)}
        </p>
      ))}
    </div>
  );
...
export function TrendLine({ data = [], isLoading }: TrendLineProps) {
  ...
  const chartData = data.map((d) => ({
    name: t.charts.gcMonths[d.month - 1],
    income: d.income,
    expense: d.expense,
  }));
  ...
  return (
    <div className="surface-1 rounded-2xl p-5">
      <p className="text-sm font-semibold mb-0.5">{t.charts.incomeVsExpenses}</p>
      <p className="text-xs text-muted-foreground mb-4">{t.charts.last6Months}</p>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={chartData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
          ...
          <Tooltip content={<CustomTooltip incomeLabel={t.charts.earned} expenseLabel={t.charts.spent} />} />
      ...
      <div className="flex items-center gap-4 mt-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-muted-foreground">{t.charts.earned}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-0.5 rounded bg-expense" />
          <span className="text-xs text-muted-foreground">{t.charts.spent}</span>
        </div>
      </div>
    </div>
  );
